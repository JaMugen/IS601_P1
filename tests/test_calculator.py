...
def calculator():
    with TemporaryDirectory() as temp_dir:
        ...
        with patch.object(
            CalculatorConfig, "log_dir", new_callable=PropertyMock
        ) as mock_log_dir, patch.object(
            CalculatorConfig, "log_file", new_callable=PropertyMock
        ) as mock_log_file, patch.object(
            CalculatorConfig, "history_dir", new_callable=PropertyMock
        ) as mock_history_dir, patch.object(
            CalculatorConfig, "history_file", new_callable=PropertyMock
        ) as mock_history_file:

            # Set return values to use paths within the temporary directory
            mock_log_dir.return_value = temp_path / "logs"
            mock_log_file.return_value = temp_path / "logs/calculator.log"
            mock_history_dir.return_value = temp_path / "history"
            mock_history_file.return_value = (
                temp_path / "history/calculator_history.csv"
            )
...
def test_perform_operation_exception_handling(calculator):
    ...
    calculator.set_operation(operation)
    with patch.object(
        Calculation, "calculate", side_effect=Exception("calculation error")
    ):
...
def test_calculator_repl_exit_save_failure_warns(mock_print, mock_input):
    ...
    with patch(
        "app.calculator.Calculator.save_history", side_effect=Exception("disk full")
    ):
        calculator_repl()
...
def test_calculator_repl_history_prints_entries(mock_print, mock_input):
    ...
    calc1 = Calculation(
        operation="Addition", operand1=Decimal("2"), operand2=Decimal("3")
    )
    calc2 = Calculation(
        operation="Multiplication", operand1=Decimal("4"), operand2=Decimal("5")
    )
...
def test_calculator_repl_decimal_normalization(mock_print, mock_input):
    ...
    with patch(
        "app.calculator.Calculator.perform_operation", return_value=Decimal("5")
    ):
        calculator_repl()
...
def test_calculator_repl_operation_unexpected_exception(operation):
    ...
    with patch(
        "app.calculator.Calculator.perform_operation", side_effect=Exception("test")
    ):
...
@patch("app.calculator_config.CalculatorConfig.get_prompt_color", return_value="green")
@patch("app.calculator_config.CalculatorConfig.get_result_color", return_value="cyan")
def test_calculator_get_prompt_and_result_colors(mock_get_result_color, mock_get_prompt_color):
    calc = Calculator()
    assert calc.get_prompt_color() == "green"
    assert calc.get_result_color() == "cyan"
